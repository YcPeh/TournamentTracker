...
namespace TrackerLibrary.DataAccess.TextHelpers
{
    public static class TextConnectorProcessor
    {
        ...
        private static string ConvertMatchupListToString(List<MatchupModel> matchcups)
        {
            string output = "";
            if (matchcups.Count == 0)
            {
                return "";
            }
            // 2^5
            foreach (MatchupModel m in matchcups)
            {
                output += $"{m.Id}^";
            }
            output = output.Substring(0, output.Length - 1);
